...
function loadProducts(page) {
    // 1. حساب قيمة 'skip' (التجاوز)
    const skip = page * limit; 
    
    // 2. بناء رابط الـ API مع limit و skip
    // هذا يطلب 12 منتجًا فقط، مع تجاوز العدد المحدد في 'skip'
    let apiUrl = currentCategory ? 
        `https://dummyjson.com/products/category/${currentCategory}?limit=${limit}&skip=${skip}` :
    ...
    fetch(apiUrl)
        ...
        .then(data => {
            ...
            // ⚠️ ملاحظة هامة: تم حذف منطق الفلترة والفرز المحليين
            // لأننا الآن نجلب 12 منتجًا فقط، ولا يمكن تطبيق الفلترة على كل المنتجات.
            
            // 3. عرض المنتجات التي جلبها الـ API مباشرةً
            displayProducts(data.products); 
            
            // 4. استخدام العدد الكلي للمنتجات (data.total) لتحديث التصفح
...
// دالة الفرز (موجودة ولكنها غير فعالة حالياً مع نظام Skip)
...
function displayProducts(products) {
    ...
    updateFavoriteIcons(); // تحديث أيقونات المفضلة بعد عرض المنتجات
}
